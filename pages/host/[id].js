...
export default function HostPage() {
  ...
  const handleTouchEnd = (e) => {
    ...
    if (Math.abs(diff) > 50) {
      const vp = visiblePhotos
      if (diff > 0) setLightboxIndex(i => Math.min(i + 1, vp.length - 1))
  ...
  const commentCount = (gid) => gid === 'all' ? comments.length : comments.filter(c => c.table === gid).length
